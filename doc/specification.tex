...
\newglossaryentry{CentOS}{
  ...
  description={An enterprise-class Linux distribution}
}
\newglossaryentry{Git}{
  type=references,
...
\newglossaryentry{GitLab}{
  ...
  description={A web-based Git-repository manager}
}
\newglossaryentry{GNU Radio}{
  type=references,
  name=\href{https://www.gnuradio.org/}{GNU Radio},
  description={An SDR development platform}
}
...
\newglossaryentry{bladeRF 2.0}{
  type=references,
  name=\href{https://www.nuand.com/bladerf-2-0-micro/}{bladeRF 2.0},
  description={An SDR device}
}
...
\newglossaryentry{RC-2SP4T-A18}{
  ...
  description={A dual programmable SP4T RF switch}
}
...
\newglossaryentry{gr-leo}{
  ...
  description={A GNU Radio space telecommunication simulator}
}
...
\newglossaryentry{CPU threads}{
  name={CPU threads},
  description={CPU concurrent execution}
}
...
\begin{itemize}
\item A computer host where the SDR Testbed software is running.
...
\item A set of programmable RF components (e.g. RF switch, attenuators, etc.).
...
\requirement{REQ.0004}{Support for PlutoSDR SDR device}{
  ...
  Based on the requirements survey, PlutoSDR is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0005}{Support for Airspy Mini SDR device}{
  The SDR testbed shall support Airspy Mini SDR device.
}{
  Based on the requirements survey, Airspy Mini is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0006}{Support for SDRPlay RSPduo SDR device}{
  The SDR testbed shall support SDRPlay RSPduo SDR device.
}{
  Based on the requirements survey, SDRPlay RSPduo is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0007}{Support for LimeSDR Mini SDR device}{
  The SDR testbed shall support LimeSDR Mini SDR device.
}{
  Based on the requirements survey, LimeSDR Mini is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0008}{Support for bladeRF 2.0 micro SDR device}{
  The SDR testbed shall support bladeRF 2.0 micro SDR device.
}{
  Based on the requirements survey, bladeRF 2.0 micro is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0009}{Support for USRP B210 SDR device}{
  The SDR testbed shall support USRP B210 SDR device.
}{
  Based on the requirements survey, USRP B210 is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0010}{Support for RTL-SDR v3 SDR device}{
  The SDR testbed shall support RTL-SDR v3 SDR device.
}{
  Based on the requirements survey, RTL-SDR v3 is one of the selected SDR devices to be supported.
}{}
\requirement{REQ.0011}{Support for programmable selection of SDR devices}{
  A pair of programmable SP4T RF switches shall be used to select RF connections between SDR devices.
  The RF switch characteristics shall be:
  \begin{itemize}
  \item isolation: >=60dB
  ...
  For each experiment a pair of SDR devices need to be selected.
...
\requirement{REQ.0012}{Support for programmable selection of attenuation levels}{
  ...
  The RF attenuator characteristics shall be:
  \begin{itemize}
...
\requirement{REQ.0013}{Fixed attenuation between selected SDR devices}{
  There shall be a fixed RF attenuation between the selected pair of SDR devices.
...
\requirement{REQ.0014}{Support for GNU Radio SDR development platform}{
  GNU Radio shall be used as the SDR development platform.
}{
  GNU Radio is the de facto standard for developing SDR applications.
}{}
\requirement{REQ.0015}{Support for gr-leo}{
  gr-leo support shall be included with GNU Radio.
}{
  gr-leo is a space telecommunication simulator.
  Channel simulation is an essential feature for an SDR Testbed focusing on satellite communications.
}{}
\requirement{REQ.0016}{Support for gr-soapy}{
  gr-soapy support shall be included with GNU Radio.
}{
  gr-soapy is an interface to SoapySDR for GNU Radio.
  It provides a clean API for configuring various SDR devices.
}{}
\requirement{REQ.0017}{Configurability of RF switches and attenuators}{
  A utility shall be developed to configure and control the SDR Testbed hardware.
  This utility shall be used to control:
  \begin{itemize}
  \item programmable RF switches
  \item programmable RF attenuators
  \end{itemize}
  This application shall utilize VISA and SCPI protocols.
  The communication with the devices shall be via Ethernet.
}{
  Programmable switches and attenuators need to be configured before the start or during the experiment.
  The testbed needs to support such a function.
}{}
\requirement{REQ.0018}{Abstraction interface to SDR application}{
  A wrapper application shall be developed to provide an abstraction layer (API) for executing SDR applications.
  The wrapper shall provide separate control and data sockets.
  The socket shall be network or Unix domain sockets.
  The control socket shall be used for controlling the SDR application.
  The data socket shall be used to exchanging data.
}{
  A form of an API is required for the SDR Testbed to execute SDR application in a consistent way.
  Additionally, the above interfaces will allow realtime control of the SDR application.
}{}
\requirement{REQ.0019}{Use of Git for submitting scenario and data}{
  Git shall be used to submit scenarios and data for an experiment.
  Scenarios, in the form of Robot Framework suites shall be committed to a repository.
  Bulk data shall be committed in Git LFS.
}{
  Git is a software version control system.
  By using Git to store scenarios and data, ATDD and HIL is possible.
}{}
\requirement{REQ.0020}{GitLab CI as a job service}{
  GitLab and GitLab CI shall be used as a job and experiment results storage service.
  The computer host shall be running a GitLab Runner.
  The runner shall be registered to GitLab.
  It shall be configured as a Docker executor.
  Parallel job execution shall not be allowed.
  Experiment results shall be upload as artifacts to GitLab.
}{
  GitLab CI is a continuous integration and delivery service.
  Such a CI service is required by SDR Testbed for job queuing and storage of experiment results.
}{}
\requirement{REQ.0021}{Support of Robot Test Automation Framework}{
  Robot Framework shall be used as an Automation Framework.
  A set of libraries shall be developed to extend the framework:
  \begin{itemize}
  \item Library for controlling the SDR Testbed configuration utility.
  \item Library for handling all functions of the SDR controller.
  \end{itemize}
  Experiment scenarios shall be written as Robot Framework Suites.
}{
  An automation framework is required to execute elaborate scenarios.
  Robot Framework is an automation framework for acceptance testing, acceptance test driven development (ATDD), and robotic process automation (RPA).
  The framework needs to be extended with SDR Testbed specific functions.
}{}
\requirement{REQ.0022}{Storing of experiment results in Hierarchical Data Format}{
  It shall be possible to store experiment results in HDF5\textsuperscript{\textregistered} format.
}{
  HDF is a file format designed to store and organize large amounts of data.
  Testbed experiment which generated large amounts of data or need to store metadata information can benefit by HDF5\textsuperscript{\textregistered} support.
}{}
\requirement{REQ.0023}{Limitation of experiment running time}{
  The amount of time an experiment can be running shall be limited.
  A timeout of 30 minutes shall be set for this purpose.
  After the time has elapsed, the execution of the experiment shall be terminated and the resources shall be freed.
}{
  The SDR Testbed has limited resources since the jobs are executed sequentially.
  A limit has to be set on the time an experiment will be running so that the testbed can be available to other users.
}{}
\requirement{REQ.0024}{Limitation of experiment results size}{
  The amount of data an experiment can produce shall be limited.
  The maximum data that an experiment can output shall be set to 100MiB.
  Data that exceed the above limit shall not be uploaded to the storage service.
}{
  The SDR Testbed and storage service has limited resources.
  A limit has to be set on the amount of data an experiment can generate.
}{}
\requirement{REQ.0025}{Containerization of SDR Testbed applications}{
  The following SDR Testbed application shall be containerized with Docker:
  \begin{itemize}
  \item Robot Framework
  \item SDR Testbed configuration utility
  \item GNU Radio
  \item SDR controller
  \end{itemize}
  A single container shall be created which will contain all the above software.
}{
  Containerizing the SDR Testbed applications provides isolation, a reproducible environment and easier management.
  It also allows the use of GitLab Runner Docker executors.
}{}
